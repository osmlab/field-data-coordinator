...
const resolveSurvey = (surveyDefinition, options, callback) => {
  const featureTypes = surveyDefinition.feature_types
  let addFeatureTypes = callback => callback(null, featureTypes)
      ...
      (err, { categories, featureTypes }) => {
        ...
        featureTypes = featureTypes.map(ft => {
          if (ft.extend != null) {
            ...
            const ftDef = featureTypes.find(x => x.id === ft.id)
            // definitions to extend
            const defs = []

            if (Array.isArray(ft.extend)) {
              defs.push(...featureTypes.filter(x => ft.extend.includes(x.id)))
            } else {
              defs.push(featureTypes.find(x => x.id === ft.extend))
            }
