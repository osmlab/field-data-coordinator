...
function importBulkOsm (xmlReadableStream, cb) {
  if (osmOrgDb) { osmOrgDb.db.close() }
  rimraf(osmOrgDbPath, function () {
    importer(osmOrgDbPath, xmlReadableStream, function (err) {
      if (err) {
        cb(err)
      } else {
        osmOrgDb = osmdb(osmOrgDbPath)
